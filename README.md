...
- Python >= 3.11, < 3.13
- [Marimo](https://marimo.io) notebook environment
...
Alternatively, each notebook carries a self-contained
[PEP 723](https://peps.python.org/pep-0723/) dependency block at the top and can
be run directly with `uv`:

```bash
uv run marimo run SRApapyrus_2/VILanalysis_auto.mo.py
```
...
│
├── docs/
│   └── figure_map.md                 — thesis figure → notebook cell mapping
│
├── SRApapyrus_2/                     — dataset SRA (papyrus support)
│   ├── VILanalysis_auto.mo.py        — Marimo analysis notebook
│   ├── *.tif                         — input TIFF images (see Data section)
│   ├── vil_outputs_figures/          — generated figures (PDF, PNG, SVG)
│   └── vil_outputs_tables/           — generated tables (CSV, TXT)
│
├── SRBpaper_2/                       — dataset SRB (paper support)
│   ├── VILanalysis_auto.mo.py        — Marimo analysis notebook (same pipeline)
│   ├── *.tif                         — input TIFF images (see Data section)
│   ├── vil_outputs_figures/
│   └── vil_outputs_tables/
│
└── Swatches_nip-workspace/
    └── SR2swatches.ws                — NIP2 workspace file for swatch extraction
```
...
TIFF input files are not included in this repository due to file size. See the
**Data** section below for access instructions. Place the TIFF files in the
corresponding dataset folder (`SRApapyrus_2/` or `SRBpaper_2/`) before running.

### 2. Run the notebook

```bash
# Interactive mode (opens in browser, recommended for exploration)
marimo run SRApapyrus_2/VILanalysis_auto.mo.py
marimo run SRBpaper_2/VILanalysis_auto.mo.py

# Headless mode (saves all outputs without opening a browser)
python SRApapyrus_2/VILanalysis_auto.mo.py
python SRBpaper_2/VILanalysis_auto.mo.py
```

Running the notebook top-to-bottom regenerates all figures and tables in
`vil_outputs_figures/` and `vil_outputs_tables/`. Existing files are overwritten.

### 3. Configuring the pipeline

All user-configurable parameters are in **Cell 0.0** of each notebook:

| Parameter | Purpose |
|---|---|
| `IMAGE_DIR` | Path to the folder containing TIFF files |
| `CONTROL_ID` | ID of the reference control swatch |
| `CHANNEL` | Colour channel to extract (0 = red; `None` = grayscale / 16-bit) |
| `HIST_XLIM` | x-axis range for KDE plots (raw pixel units) |
| `ZSCORE_THRESHOLD` | Outlier-exclusion threshold (default 2.0 σ) |
| `SWATCH_REGISTRY` | List of all swatches with ID, filename, label, and group |
| `AGEING_PAIRS` | Pairs of unaged/aged TIFF filenames for before/after comparison |

See `docs/figure_map.md` for a full mapping of thesis figures to notebook cells.
...
Each dataset folder contains TIFF images of individual mockup swatches imaged
under VIL illumination (visible excitation, near-infrared detection):

- **8-bit RGB TIFFs** — red channel (channel 0) extracted; values 0–255
- Naming convention: `{SwatchID}.tif` (unaged) and `{SwatchID}_aged.tif` (after ageing)
...
The TIFF image files are available on request from the author. They are not
distributed in this repository due to file size constraints. Please contact
[your institutional email] or open a GitHub issue.
...
- **Agnes Brokerhof** — (Rijksdienst voor het Cultureel Erfgoed, RCE) institutional support and equipment. Namely, VIL capturing device: Crime-lite AUTO ( foster+freeman, the).
- **Dina Danish & Jean-Baptise Maitre** - amsterdam-based artists and researchers, for the generous donation of their own synthesised Egyptian Blue(sample F5EG2, preparation 24.10.2025, firing 18.11.2025). Publications forthcoming.
- Coding assistance was additionally provided by Anthropic Claude Sonnet 4.6
  (repository preparation, code assistance). See `TRANSPARENCY.md` for full details.
